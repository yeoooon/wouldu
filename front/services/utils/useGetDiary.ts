...
export const useGetDiary = (date: string) => {
  ...
  useEffect(() => {
    ...
    if (data?.diaries?.find((el: any) => el.userId === user?.id)) {
      setUserDiary({
        title: data?.title!,
        authorId: data?.diaries?.find((el: any) => el.userId === user?.id)!.authorId,
        id: data?.diaries?.find((el: any) => el.userId === user?.id)!.id,
        nickname: data?.diaries?.find((el: any) => el.userId === user?.id)!.user.nickname,
        content: data?.diaries?.find((el: any) => el.userId === user?.id)!.content,
      });
    } else {
      setUserDiary(undefined);
    }
